...
def upsample(in_tens, out_HW=(64, 64)):  # assumes scale factor is same for H and W
    ...
    return nn.Upsample(size=(out_HW[0], out_HW[1]), mode='bilinear', align_corners=False)(in_tens)
...
class LPIPS(nn.Module):
    def __init__(self, pretrained=True, net='alex', version='0.1', lpips=True, spatial=False,
        ...
        self.adaptive_perceps = []
        for chn in self.chns:
            adaptive_percep = AdaptiveLossFunction(
                num_dims=chn, float_dtype=np.float32, device=0)
            self.adaptive_perceps.append(adaptive_percep)
    ...
    def forward(self, in0, in1, use_robust, retPerLayer=False, normalize=False):
        ...
        for kk in range(self.L):
            ...
            bz, ch, h, w = feats0[kk].shape
            if use_robust:
                diffs[kk] = self.adaptive_perceps[kk].lossfun(
                    feats0[kk].permute(0, 2, 3, 1).reshape(-1, ch) - feats1[kk].permute(0, 2, 3, 1).reshape(-1, ch))
                diffs[kk] = diffs[kk].reshape(bz, h, w, ch).permute(0, 3, 1, 2)
            else:
                diffs[kk] = (feats0[kk] - feats1[kk]) ** 2
            # diffs[kk] = loss(feats0[kk].permute(0,2,3,1).reshape(-1, ch) - feats1[kk].permute(0,2,3,1).reshape(-1, ch))
        self.diffs = diffs
        if (self.lpips):
            if (self.spatial):
