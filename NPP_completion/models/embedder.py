...
def get_embedder(multires,  i=0, res=None, selected_angles=None, selected_periods=None, freq_scales=None, freq_offsets=None, angle_offsets=None):
    if i == -1:
            ...
            for freq_offset in self.freq_offsets:
                for idx in range(len(selected_angles)):
                    for angle_offset in self.angle_offsets:
                        ...
                        #     shifted_angle = selected_angle - 90
                        #     theta_r = torch.deg2rad(shifted_angle)
                        #     embed_fns = lambda x, y, freq=freq, theta_r=theta_r: (((x - y * torch.tan(theta_r)) % (freq / torch.cos(theta_r)) / (freq / torch.cos(theta_r))) - 0.5 ) * 2
                        #     if idx == 0:
                        ...
                        #
                        # else:
                        #     shifted_angle = selected_angle
                        #     theta_r = torch.deg2rad(shifted_angle)
                        #
                        #     embed_fns = lambda x, y, freq=freq, theta_r=theta_r: (((y + x * torch.tan(theta_r)) % (freq / torch.cos(theta_r)) / (freq / torch.cos(theta_r))) - 0.5 ) * 2
                        #
                        #     if idx == 0:
                        #         embed_fns_x.append(embed_fns)
                        #     else:
                        ...
                        embed_fns = lambda x, y, freq=freq, theta_r=theta_r: (  ( (y * torch.cos(theta_r) + x * torch.sin(theta_r)) % freq ) / freq - 0.5  ) * 2
