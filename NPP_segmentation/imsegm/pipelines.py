...
from .graph_cuts import estim_class_model, segment_graph_cut_general
from .labeling import histogram_regions_labels_norm
from .superpixels import segment_slic_img2d, segment_slic_img3d_gray
from .utilities import ImageDimensionError
from .utilities.experiments import get_nb_workers, WrapExecuteSequence
...
def estim_model_classes_group(
    ...
    _wrapper_compute = partial(
        compute_color2d_superpixels_features, sp_size=sp_size, sp_regul=sp_regul, dict_features=dict_features, mask=mask
    )
...
def segment_color2d_slic_features_model_graphcut(
    ...
    slic, features = compute_color2d_superpixels_features(image, dict_features, mask=mask, sp_size=sp_size, sp_regul=sp_regul)
    ...
    # slic = (slic * (mask) +   1-mask).astype(np.int64)
    # slic = slic
    # slic = (slic * (mask) ).astype(np.int64)
    segm_soft = proba[slic-1]
    ...
    segm = graph_labels[slic-1]
    return segm, segm_soft


def compute_color2d_superpixels_features(image, dict_features, mask=None, sp_size=30, sp_regul=0.2):
    ...
    slic = segment_slic_img2d(image, sp_size=sp_size, relative_compact=sp_regul, mask=mask)
